  ...
  Widget build(BuildContext context) {
    ...
    return Listener(
      onPointerDown: (_) {
        ...
        if (state.inputAllowed) state.executeConsequence(state.scenes.active.backgroundConsequence);
      },
      child: AspectRatio(
        aspectRatio: 1.0,
        child: Container(
          ...
          child: state.playAreaVisible
              ? GridView.count(
                  ...
                  crossAxisCount: 3,
                  children: [
                    for (var t in state.scenes.active.targets)
                      Transform.translate(
                        offset: Offset(
                          (rng.nextDouble() - 0.5) * state.scenes.active.positionNoise * 25.0,
                          (rng.nextDouble() - 0.5) * state.scenes.active.positionNoise * 25.0,
                        ),
          ...
          child: Container(
              ...
              child: UnconstrainedBox(
                child: target.alpha > 0
                    ? SizedBox(
                        width: state.scenes.active.targetSize * 10,
                        height: state.scenes.active.targetSize * 10,
                        child: Container(
